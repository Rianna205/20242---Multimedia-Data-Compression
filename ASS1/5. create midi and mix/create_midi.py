...
for chord, dur in left_hand_events:
    note_ons = []
    note_offs = []
    # Check if it's a single note or a full chord
    if len(chord) == 1:
        # For a single (bass) note, lower by one octave (-12 semitones)
        for note in chord:
            note_number = get_midi_note(note) - 12
        ...
        # For multiple-note chords, use the standard pitch (no octave change)
        for note in chord:
            note_number = get_midi_note(note)
            note_ons.append(Message('note_on', note=note_number, velocity=70, time=0))
            note_offs.append(Message('note_off', note=note_number, velocity=64, time=0))
    
    # Add the note on/off messages to the track.
    if note_ons:
        note_ons[0].time = 0  # First note gets the current delta-time
